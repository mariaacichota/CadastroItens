...
begin
  ...
  try
    ...
    FDQuery.SQL.Text := 'DELETE FROM tab_orders_item WHERE order_id = :id; DELETE FROM tab_orders WHERE order_id = :id; ';
...
begin
  ...
  try
    ...
    begin
      SQL.Text := 'INSERT INTO tab_orders (client_id, order_date, total_value) VALUES (:ClientID, :OrderDate, 0)';
...
begin
  ...
  begin
    Append;
    FieldByName('order_id').AsInteger := OrderId;
    FieldByName('item_id').AsInteger := ProductId;
    FieldByName('quantity').AsInteger := Quantity;
    FieldByName('unit_price').AsFloat := UnitPrice;
    Post;
  end;
end;
